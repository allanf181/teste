...
function conv($string){
    ...
    if(strlen($string) == 1)
        ...
        for($i = 0; $i < strlen($string); $i++){
            ...
            if ($p==194 && $q==186) $p=186; //º;
            if ($p==195 && $q==135) $p=199; //&Ccedil;
            if ($p==195 && $q==167) $p=199; //&Ccedil;

            if ($p==195 && $q==160) $p=192; //&Aacute;
            if ($p==195 && $q==173) $p=205; //&Iacute;

            if ($p==206 && $q==147) $p=205; //&Iacute;
            
            if ($p==190) $p=192; //&Agrave;;

            if ($p==226 && $q==149){       // 3 bytes
                if ($r == 152) $p=193;     //&Aacute;   
                if ($r == 153) $p=192;     //&Agrave;
                if ($r == 171) $p=195;     //&Atilde;
                if ($r == 147) $p=194;     //&Acirc;
                $i++;
            }
            ...
            if ($p==226 && $q== 137){      // 3 bytes
                if ($r == 136) $p=213;      //&Otilde
                $i++;
            }
            if ($p==226 && $q==140){        // 3 bytes
                if ($r==160) $p=195;        //&Atilde;
                $i++;
            }
            if ($p==195 && $q==161) $p=193; //&Aacute;
            if ($p==195 && $q==179) $p=211; //&Oacute;
            if ($p==206 && $q==166) $p=211; //&Oacute;
            if ($p==206 && $q==166) $p=212; //&Ocirc;
            if ($p==206 && $q==152) $p=212; //&Ocirc;
            if ($p==206 && $q==181) $p=218; //&Uacute;
            if ($p==206 && $q==169) $p=213; //&Otilde;
            if ($p==195 && $q==169) $p=201; //&Eacute;
            if ($p==195 && $q==170) $p=202; //&Ecirc;
            if ($p==195 && $q==137) $p=201; //&Eacute;
            if ($p==195 && $q==163) $p=211; //&Oacute;

            if ($a==199 && $p==194) $p=195; //&Atilde;
            if ($q==79 && $p==194) $p=195; //&Atilde;
			if ($p==204) $p=205; //&Iacute;

			if ($q==78 && $p==193) $p=194; //&Acirc
			if ($p==231 && $q==97) $p=199; //&Ccedil;;
			if ($p==225 && $q==112) $p=193; //&Aacute;
			
			// NAYLOR SCRIPT CRON
            if ($a!=32 && $q!=32 && $p==200) $p=202; //&Eacute;
            if ($a!=32 && $q!=32 && $p==192) $p=193; //&Aacute;

			if (!$c && $p==194 && $q==149) { $p=193;$c=1; } //&Aacute;
			if (!$c && $p==206 && $q==63)  { $p=218;$c=1; } //&Uacute;
			if (!$c && $p==195 && $q==180) { $p=212;$c=1; } //&Ocirc;
			if (!$c && $p==195 && $q==149) { $p=195;$c=1; } //&Aacute;
			if (!$c && $p==193 && $q==149) { $p=193;$c=1; } //&Atilde;
			if (!$c && $p==195 && $q==186) { $p=218;$c=1; } //&Atilde;
		
			if ($r==147 && $p==193 && $q==149) $p=194; //&Acirc;
 
			if ($p==149 && $q==152) $jump=1;
			if ($p==149 && $q==171) $jump=1;
			if ($p==92 && $q==39) $jump=1;
			if ($p==63 && $q==78) $jump=1;
			if ($p==149 && $q==147) $jump=1;
  
            // QUANDO O SCRIPT É EXECUTADO PELO CRON
            if ($p==137 || $p==147 || $p==135 || $p==179 
             || $p==152 || $p==161 || $p==160 || $p==180
             || $p==170 || $p==169 || $p==186 || $p==167
             || $p==162 || $p==171 || $p==136 || $p==166
             || $p==173 || $p==181 || $p==140 || $p==153
             ) $jump=1;
...
function situacaoNovo(){
    ...
    $sql = "insert into Situacoes (codigo, nome, sigla, listar, habilitar) values(2,'Cancelado', 'CD', 1, 0);";
   	$res = mysql_query($sql);
    $sql = "insert into Situacoes (codigo, nome, sigla, listar, habilitar) values(3,'Aprovado', 'AP', 1, 0);";
   	$res = mysql_query($sql);
    $sql = "insert into Situacoes (codigo, nome, sigla, listar, habilitar) values(4,'Reprovado', 'RP', 1, 0);";
   	$res = mysql_query($sql);
	...
	$sql = "insert into Situacoes (codigo, nome, sigla, listar, habilitar) values(6,'Trancado', 'TR', 1, 0);";
   	$res = mysql_query($sql);
	$sql = "insert into Situacoes (codigo, nome, sigla, listar, habilitar) values(7,'Retido pelo Modulo', 'RM', 1, 0);";
   	$res = mysql_query($sql);
	$sql = "insert into Situacoes (codigo, nome, sigla, listar, habilitar) values(8,'Aprovado pelo Modulo', 'AM', 1, 0);";
   	$res = mysql_query($sql);
	$sql = "insert into Situacoes (codigo, nome, sigla, listar, habilitar) values(9,'Aprovado pelo Conselho', 'AC', 1, 0);";
   	$res = mysql_query($sql);
...
function modalidadesCursoAntigo(){
  ...
  while ($r = db2_fetch_object($res2)){
  	if (!is_int($r->MD_MODAL)) $r->MD_MODAL += (ord($r->MD_MODAL) + 2000); // PARA BASE DE SP...
