...
for ($n=1; $n <= 2; $n++) {
	...
	while ($row = db2_fetch_object($res)) {
	  ...
	  $sql = "SELECT * FROM Pessoas p, PessoasTipos pt 
	    			...
	    			AND p.prontuario='$row->PR_PRONTP' AND pt.tipo = $PROFESSOR";

	  $result = mysql_query($sql);
    $professor = mysql_fetch_object($result);
    $sexo=0;

	  if ($row->PR_SEXO && $row->PR_SEXO != ' ')
   		$sexo = $sexos[strtoupper($row->PR_SEXO)];
...
if (!$LOCATION_CRON) {
	...
	?>
	<script>
  		$('#db2ProfessoresRetorno').text('<?php print $i; ?> importados, <?php print $j; ?> atualizados');
	</script><?php
} else {
