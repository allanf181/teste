...
$db2_2 = "SELECT GRD_CURSO,GRD_MODAL,GRD_DISC,GR_SEMANA 
...
while ($row_2 = db2_fetch_object($res_2)) {
	$GRD[$row_2->GRD_CURSO][$row_2->GRD_MODAL][trim($row_2->GRD_DISC)] = $row_2->GR_SEMANA;
}
...
while ($row = db2_fetch_object($res)) {
	...
	if (!$curso) {
   	if (!is_int($row->T_MODAL)) $row->T_MODAL += (ord($row->T_MODAL) + 2000); // PARA BASE DE SP...
