...
while ($row = db2_fetch_object($res)) {
	...
	for ($n=1; $n <= 2; $n++) {
	    $sql = "select * from Turmas where numero='$numero' and curso=$cCodigo AND ano=$ano AND semestre = $n ORDER BY codigo ASC ";
...
if (!$LOCATION_CRON) {
	...
	?>
	<script>
  		$('#db2TurmasRetorno').text('<?= $i ?> registros processados...');
	</script><?php
} else {
