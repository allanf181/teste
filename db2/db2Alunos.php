...
while ($r = mysql_fetch_object($result)) {
	$sexos[strtoupper($r->nome[0])] = $r->codigo;
}
...
while ($row = db2_fetch_object($res)) {
    ...
    $sexo = $sexos[strtoupper($row->AL_SEXO)];
    ...
    if (empty($aluno)) { // NÃO EXISTE, ENTÃO IMPORTA
    	...
    	if (    strcmp(addslashes($aluno->nome),$nome) != 0
    	     || strcmp(addslashes($aluno->rg),$rg) != 0
    	     || strcmp(addslashes($aluno->cpf),$cpf) != 0
    	     || strcmp($aluno->email,$email) != 0
    	     || strcmp(addslashes($aluno->observacoes), $observacoes) != 0
    	     || strcmp(addslashes($aluno->endereco),$endereco) != 0
    	     || strcmp(addslashes($aluno->bairro),$bairro) != 0
    	     || strcmp($aluno->nascimento,$nascimento) != 0
    	     || strcmp($aluno->naturalidade,$naturalidade) != 0
    	     || strcmp(addslashes($aluno->cep),$cep) != 0
    	     || $aluno->cidade != $cidade
    	     || strcmp(addslashes($aluno->telefone),$telefone) != 0
    	     || strcmp(addslashes($aluno->celular),$celular) != 0
    	     || $aluno->sexo != $sexo
    	     || strcmp(addslashes($aluno->ano1g),$ano1g) != 0
    	     || strcmp(addslashes($aluno->escola1g),$escola1g) != 0
    		) {
    			$sql = "UPDATE Pessoas
					SET nome = '$nome',
	    			rg = '$rg',
...
if (!$LOCATION_CRON) {
	...
	?>
	<script>
		$('#db2AlunosRetorno').text('<?php print $i; ?> importados, <?php print $j; ?> atualizados');
	</script><?php
} else {
