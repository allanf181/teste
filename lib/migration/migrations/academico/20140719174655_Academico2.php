...
class Academico2 extends Ruckusing_Migration_Base {
    ...
    public function up() {
        ...
        // TIPOS
        $result = $this->select_all("SELECT permissao FROM Permissoes WHERE tipo = (SELECT adm FROM Instituicoes LIMIT 1)");
        if ($result) {
            if (substr($result[0]['permissao'], 0, 4) != 'view') {
