...
if ($_GET['pagina'] == "planoEnsino") {
    if ($_POST["opcao"] == 'InsertOrUpdate') {
        ...
        if (mysql_num_rows($result) > 0) {
            ...
            while ($row = mysql_fetch_assoc($result)) {
                if (substr($row["Type"], 0, 3) == "var" || substr($row["Type"], 0, 3) == "tex")
                    $ap = "'";
                else
                    $ap = '';
                if (!$$row["Field"] && (substr($row["Type"], 0, 3) == "int" || substr($row["Type"], 0, 5) == "float"))
                    $value = 'NULL';
                else
                    $value = $$row["Field"];
                $SQLins .= $ap . $value . $ap;
...
<div class="tab_container" id="form">
    ...
    if ($_GET['pagina'] == "entregarPlano") {
        ...
        if (!$BLOQ)
            print "<br><input type=\"submit\" $disabled value=\"Entregar\" id=\"Entregar\"></th>";
        else
...
if ($_GET['pagina'] == "planoAula") {
    ...
    $disabled = '';
    $resultado = mysql_query("SELECT pe.finalizado, pe.codigo
    ...
    <table id="listagem" border="0" align="center">
        ...
        while ($linha = mysql_fetch_array($resultado)) {
            ...
            if (!$$disabled)
                print "<td align='center'><a href='#' title='Excluir' class='item-excluir' id='" . crip($linha[0]) . "'><img class='botao' src='" . ICONS . "/remove.png' /></a><a href='#' title='Alterar' class='item-alterar' id='" . crip($linha[0]) . "'><img class='botao' src='" . ICONS . "/config.png' /></a></td>\n";
            else
                print "<td>&nbsp;</td>\n";
            print "</tr>";
            $i++;
