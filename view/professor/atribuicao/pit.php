...
if ($_POST) {
    if (!dcrip($_POST['area'])) {
        print "<span style='font-weight: bold; color: red'>Atenção, é necessário cadastrar uma FPA primeiro.</span>";
...
if ($_GET['pano'] && $_GET['psemestre']) {
    ...
    $params = array('ano' => $pano, 'semestre' => $psemestre, 'pessoa' => $_SESSION['loginCodigo'], 'modelo' => 'PIT');
    ...
    $params = array('pessoa' => $_SESSION['loginCodigo'], 'modelo' => 'PIT');
    $res = $dados->listModelo($params, $sqlAdicional, null, null);
...
if (!$resC){ // PIT AINDA NAO FOI CADASTRADA, BUSCANDO DADOS DA FPA
    $params['modelo'] = 'FPA';
    $res2 = $dados->listModelo($params, $sqlAdicional, null, null);
    $codigo = $res2[0]['codigo'];
    $resC = $componente->listComponentes($codigo);
    if ($resC)
        echo "<script>msg('Os dados de sua FPA foram importados. Preencha os horários, atualize o que for necessário e Salve ou Envie para avaliação do coordenador.')</script>";
}
//LISTA ATIVIDADES
...
<center>
    <div id="html5form" class="main">
        <form id="form_padrao">
            ...
            <font size="3"><b>PIT - PLANO INDIVIDUAL DE TRABALHO DOCENTE <br> <?= $psemestre ?>&ordm; semestre <?= $pano ?> </b></font>
    ...
    if (!$resFPA || (is_null($resFPA[0]['valido']))){
        mensagem('ERRO', 'FPA_NAO_VALIDADA');        
    }
    else{
                    ...
                    for ($p = 1; $p <= 3; $p++) {
                        ...
                        <tr align="center">
                            ...
                            for ($i = 1; $i <= 36; $i++) { // LINHAS DA TABELA
                                ...
                                $IS = $p . $l . $c;
                                ?>
                                <td><input id="CE<?= $IS ?>" <?= $disabled ?> name="horario[<?= $IS ?>]" value="<?=$horarios[$n]?>" type="text" size="10" maxlength="45" class="horario" onclick="return valores('horarios', 'CE<?= $IS ?>');" /></td>
                                <?php
                                $c++;
                                $n++;
                            }
...
<script>
    ...
    function calcHorario(){
        ...
        if (!preenchido)
            $("#obsCH").html('Horário consolidado não foi preenchido.');
    }

    $(".horario,.componente,.atividade,.complementacao,#Periodo1,#Periodo2,#Periodo3").on('keyup change blur',function () {
        callFunction();
    });
