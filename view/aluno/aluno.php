...
if ($_GET["atribuicao"]) {
    ...
    <table width="100%" align="center" border="0">        
        <tr align='center'>
            ...
            <td valign="top" width="90"><a class='nav professores_item' href="javascript:$('#aluno').load('<?= VIEW ?>/aluno/chat.php?atribuicao=<?= crip($atribuicao) ?>&codDisciplina=<?= crip($res['codDisciplina']) ?>&origem=<?= ($bolsista)?crip("bolsista"):"" ?>&timestamp=<?= time() ?>');void(0);"><div id="imageChat"><img style='width: 60px' src='<?= INC ?>/file.inc.php?type=chat&atribuicao=<?= crip($atribuicao) ?>&origem=<?= ($bolsista)?crip("bolsista"):"" ?>' /></div>Chat (Atendimento)</a></td>
            <td valign="top" width="90"><a class='nav professores_item' href="javascript:$('#aluno').load('<?= VIEW ?>/aluno/atvAcadEmica.php?aluno=<?= crip($aluno) ?>');void(0);"><img style='width: 60px' src='<?= IMAGES ?>/atvAcadEmicas.png' /><br />Atividades Acad&ecirc;micas</a></td>

            <td valign="top" width="90"><a class='nav professores_item' href="javascript:$('#aluno').load('<?= VIEW ?>/aluno/boletim.php?turma=<?= crip($res['turmaCodigo']) ?>&aluno=<?= crip($aluno) ?>&bimestre=<?= crip($numeroBimestre) ?>');void(0);"><img style='width: 60px' src='<?= IMAGES ?>/boletim.png' /><br />Boletim Escolar</a></td>
            <?php
            if ($bimestre == "ANUAL" || $bimestre == "SEMESTRAL" || $bimestre == "1&ordm; BIMESTRE") {
                ?>
                <td valign="top" width="90"><a class='nav professores_item' href="javascript:$('#aluno').load('<?= VIEW ?>/aluno/planoEnsino.php?atribuicao=<?= crip($atribuicao) ?>');void(0);"><img style='width: 60px' src='<?= IMAGES ?>/planoEnsino.png' /><br />Plano de Ensino</a></td>
            <?php } ?>
        </tr>
        <tr>
