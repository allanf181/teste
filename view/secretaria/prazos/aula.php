...
if (isset($_GET["professor"]) && dcrip($_GET["professor"]) != 'Todos') {
  ...
  $restricao .= " AND p.professor = $professor";
    ...
    <table align="center" id="form" width="100%" border="0">
		...
		<td rowspan="2"><input type="submit" name="liberar" value="Liberar">
		</tr>
            </td></tr>
	...
	$sql = "select a.codigo, d.nome, d.codigo, t.numero, DATEDIFF(a.prazo,NOW()), a.bimestre, a.prazo, p.professor
	...
	$('#form_padrao').on('submit', function(e){
      ...
      professor = $('#campoProfessor').val();
	    ...
	    jPrompt(modo, '', '<?php print $TITLE; ?>', function(r)
	    {
			...
			$('#index').load('<?php print $SITE; ?>?opcao=controlePrazo&prazo='+prazo+'&curso='+curso+'&turma='+turma+'&bimestre='+bimestre+'&valor1='+r+'&professor='+professor);
		    }
	    });
	});
    });
