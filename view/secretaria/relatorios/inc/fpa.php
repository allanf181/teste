...
if (dcrip($_GET["professor"])) {
    ...
    $sqlAdicional .= " AND (v.finalizado <> '' AND v.finalizado <> '0000-00-00 00:00:00') ";

    $res = $dados->listFPA($params, null, null, $sqlAdicional);
    if ($res) {
        foreach ($res as $reg) {
            $codigo = $reg['codigo'];
            $nome = $reg['nome'];
            ...
            $pdf->Cell(65, 5, utf8_decode("FPA (Anexo I - Resolução nº 112 de 7 outubro de 2014)"), 0, 0, 'C', false);
            $pdf->Ln();
            $pdf->Ln();
