...
if ($_GET["opcao"] == 'change') {
    ...
    if ($ret['STATUS'] == 'OK') {
        ...
        if ($email = $pessoa->getEmailFromPessoa($resPessoa[0]['pessoa']))
            $logEmail->sendEmailLogger($_SESSION['loginNome'], 'Foi solicitada uma corre&ccedil;&atilde;o em sua PIT.', $email);
    }    
}
...
if ($_GET["opcao"] == 'controle') {
    ...
    if ($_GET["conferido"] != 'false' && !$_GET["solicitacao"]) {
        ...
        $params['solicitacao'] = 'PIT Validada';
    } else {
        $params['solicitacao'] = 'PIT aberta para alteração.';
        $_GET["valido"] = null;
    }

    $params['nomeTabela'] = 'PIT';
...
<script>
    ...
    $(".search").click(function (event) {
        ...
        new $.Zebra_Dialog('<strong>Hist&oacute;rico de Solicita&ccedil;&otilde;es</strong>', {
            source: {'iframe': {
                    'src': 'view/secretaria/atribuicao_docente/pit.php?opcao=historico&codigo=' + codigo,
                    'height': 300
                }},
            width: 600,
            title: 'PIT'
    ...
    function Change(codigo, nome) {
        $.Zebra_Dialog('<strong>Confirma a solicitação de correção na PIT de ' + nome + '? <br><br>Descrever a solicitação:</strong>', {
            'type': 'prompt',
    ...
    function conf(checked, nome, codigo) {
        if (!checked)
            modo = '<strong>Confirma abrir a PIT de ' + nome + '?</strong>';
        else
            modo = '<strong>Confirma a conferência da PIT de ' + nome + '?<br><br>Atenção: somente o GED poderá abrir novamente!</strong>';
