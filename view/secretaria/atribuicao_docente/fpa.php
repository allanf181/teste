...
if ($_GET["opcao"] == 'change') {
    ...
    if ($ret['STATUS'] == 'OK') {
        ...
        if ($email = $pessoa->getEmailFromPessoa($resPessoa[0]['pessoa']))
            $logEmail->sendEmailLogger($_SESSION['loginNome'], 'Foi solicitada uma corre&ccedil;&atilde;o em sua FPA.', $email);
    }
}
...
if ($_GET["opcao"] == 'controle') {
    ...
    if ($_GET["conferido"] != 'false' && !$_GET["solicitacao"]) {
        ...
        $params['solicitacao'] = 'FPA Validada';
    } else {
        $params['solicitacao'] = 'FPA aberta para alteração.';
        $_GET["valido"] = null;
        $_GET["finalizado"] = null;
    }

    $params['nomeTabela'] = 'FPA';
...
<table id="listagem" border="0" align="center" width="100%">
    ...
    foreach ($res as $reg) {
        ...
        <tr <?= $cdif ?>>
            <td><?= $i ?></td>
            <td>
                <a target='_blank' href='<?= VIEW ?>/secretaria/relatorios/inc/fpa.php?professor=<?= crip($reg['pessoa']) ?>'><?= mostraTexto($reg['nome']) ?></a>
            </td>
...
<script>
    ...
    $(".search").click(function (event) {
        ...
        new $.Zebra_Dialog('<strong>Hist&oacute;rico de Solicita&ccedil;&otilde;es</strong>', {
            source: {'iframe': {
                    'src': 'view/secretaria/atribuicao_docente/fpa.php?opcao=historico&codigo=' + codigo,
                    'height': 300
                }},
            width: 600,
            title: 'FPA'
    ...
    function change(codigo, nome) {
        $.Zebra_Dialog('<strong>Confirma a solicitação de correção na FPA de ' + nome + '? <br><br>Descrever a solicitação:</strong>', {
            'type': 'prompt',
    ...
    function conf(checked, nome, codigo) {
        if (!checked)
            modo = '<strong>Confirma abrir a FPA de ' + nome + '?</strong>';
        else
            modo = '<strong>Confirma a conferência da FPA de ' + nome + '?<br><br>Atenção: somente o GED poderá abrir novamente!</strong>';
