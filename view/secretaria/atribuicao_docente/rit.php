...
if ($_GET["opcao"] == 'change') {
    ...
    if ($ret['STATUS'] == 'OK') {
        ...
        if ($email = $pessoa->getEmailFromPessoa($resPessoa[0]['pessoa']))
            $logEmail->sendEmailLogger($_SESSION['loginNome'], 'Foi solicitada uma corre&ccedil;&atilde;o em sua RIT.', $email);
    }    
}
...
if ($_GET["opcao"] == 'controle') {
    ...
    if ($_GET["conferido"] != 'false' && !$_GET["solicitacao"]) {
        ...
        $params['solicitacao'] = 'RIT Validada';
    } else {
        $params['solicitacao'] = 'RIT aberta para alteração.';
        $_GET["valido"] = null;
    }

    $params['nomeTabela'] = 'RIT';
...
<table id="listagem" border="0" align="center" width="100%">
    ...
    foreach ($res as $reg) {
        ...
        <tr <?= $cdif ?>>
            <td><?= $i ?></td>
            <td>
                <a target='_blank' href='<?= VIEW ?>/secretaria/relatorios/inc/rit.php?professor=<?= crip($reg['pessoa']) ?>'><?= mostraTexto($reg['nome']) ?></a>
            </td>
...
<script>
    ...
    $(".search").click(function (event) {
        ...
        new $.Zebra_Dialog('<strong>Hist&oacute;rico de Solicita&ccedil;&otilde;es</strong>', {
            source: {'iframe': {
                    'src': 'view/secretaria/atribuicao_docente/rit.php?opcao=historico&codigo=' + codigo,
                    'height': 300
                }},
            width: 600,
            title: 'RIT'
    ...
    function Change(codigo, nome) {
        $.Zebra_Dialog('<strong>Confirma a solicitação de correção na RIT de ' + nome + '? <br><br>Descrever a solicitação:</strong>', {
            'type': 'prompt',
            'title': '<?php print $TITLE; ?>',
            'buttons': ['Sim', 'Não'],
            'onClose': function (caption, valor) {
                if (caption == 'Sim') {
                    $('#index').load('<?php print $SITE; ?>?opcao=change&codigo=' + codigo + '&solicitacao=' + encodeURIComponent(valor));
                }
            }
        });
    }
    ...
    function conf(checked, nome, codigo) {
        if (!checked)
            modo = '<strong>Confirma abrir a RIT de ' + nome + '?</strong>';
        else
            modo = '<strong>Confirma a conferência da RIT de ' + nome + '?<br><br>Atenção: somente o GED poderá abrir novamente!</strong>';
        ...
        $.Zebra_Dialog(modo, {
            ...
            'onClose': function (caption) {
                if (caption == 'Sim') {
                    $('#index').load('<?php print $SITE; ?>?opcao=controle&codigo=' + codigo + '&conferido=' + checked);
                } else {
