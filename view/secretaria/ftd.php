...
if (in_array($ADM, $_SESSION["loginTipo"]) 
	|| in_array($COORD, $_SESSION["loginTipo"]) 
    ...
    while ($linha = mysql_fetch_array($resultado)) {
       	...
       	$correcao=0;
       	$bloqueado='';
    		if ($linha[2] == '' || $linha[2] == '00/00/0000 00:00') $linha[2] = 'NÃO';
    		$valido = $linha[3];
    		$solicitacao = $linha[4];
        $i%2==0 ? $cdif="class='cdif'" : $cdif="";
				  echo "<tr $cdif><td>$i</td>";
	        echo "<td><a target='_blank' href='relatorios/ftd.php?detalhada=1&professor=".crip($linha[6])."'>".mostraTexto($linha[1])."</a></td>";
					print "<td>$linha[2]</td>";
	      ...
	      if ($valido != "00/00/0000 00:00" && $valido != "") {
	      	...
	      	echo "<td align='center' colspan='3'><a href='#' class='tooltip' title='Correção solicitada por $linha[5]'>Correção solicitada</a></td>";
	      	$correcao=1;
	      } else {
	      	if (in_array($ADM, $_SESSION["loginTipo"]) || in_array($GED, $_SESSION["loginTipo"]) || in_array($COORD, $_SESSION["loginTipo"]))
		      	echo "<td align='center'><a href='#' class='tooltip' title='Solicitar correção' onclick=\"return FTD('".$linha[0]."', '".$linha[1]."')\";><img class='botao campoCorrecao' id='".crip($linha[0])."' name='$linha[0]' src='".ICONS."/cancel.png' /></a></td>";
				}
