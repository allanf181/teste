...
//Permite realizar a migração dos dados do antigo sistema BRT Acadêmico ao sistema atual Web Diário.
...
//                        echo $sql;
    ...
    while ($linha = mysql_fetch_array($resultado)) {
        ...
        if ($linha[4] != "0")
            $grupo = " $linha[4]";
        if ($linha[4] < 1000 || $linha[4] >= 2000)
            $linha[2] = "$linha[2] $grupo [$linha[3]ºBim]";
        echo "<option $selected value='" . crip($linha[0]) . "'> $linha[1] $linha[2]</option>";
        $selected = "";
    }
    ?>
