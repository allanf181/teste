...
if ($_POST["opcao"] == 'InsertOrUpdate') {
    ...
    $ret = $bolsaDisciplina->insertOrUpdate($_POST);
...
if ($_GET["opcao"] == 'delete') {
    $ret = $bolsaDisciplina->delete($_GET["codigo"]);
    mensagem($ret['STATUS'], $ret['TIPO'], $ret['RESULTADO']);
    $_GET["codigo"] = null;
}
...
if (!empty($_GET["codigo"])) { // se o parâmetro não estiver vazio
    ...
    $res = $bolsaDisciplina->listRegistros($params1);
    ...
    <form id="form_padrao">
        <table align="center" width="100%" id="form">
            ...
            <tr>
                ...
                <td>
                    <select name="disciplina" id="disciplina" value="<?= $disciplina ?>">
                        <option></option>
                        <?php
                        require CONTROLLER . '/disciplina.class.php';
                        $disc = new Disciplinas();
                        foreach ($disc->listDisciplinas($params, ' ORDER BY c.nome,d.numero,d.nome ') as $reg) {
                            $selected = "";
                            if ($reg['disciplina'] == $disciplina)
                                $selected = "selected";
                            print "<option $selected value='" . crip($reg['codigo']) . "'>[" . $reg['curso'] . "] [" . $reg['numero'] . "] " . $reg['disciplina'] . "</option>";
                        }
                        ?>
                    </select>
                </td>
...
$sqlAdicional .= ' ORDER BY b.dataInicio DESC, b.titulo ASC, d.nome ASC ';

$res = $bolsaDisciplina->listDisciplinas($params, $sqlAdicional, $item, $itensPorPagina);
$totalRegistros = count($bolsaDisciplina->listDisciplinas($params, $sqlAdicional, null, null));
...
<script>
    ...
    $(document).ready(function () {
        valida();
        $('#bolsa,#disciplina').change(function () {
            valida();
        });
